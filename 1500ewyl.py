...
priority_cases_sheet = client.open_by_url(priority_cases_url).worksheet('EWYL')
priority_cases_df = pd.DataFrame(priority_cases_sheet.get_all_records())

# Read the Mastersheet
mastersheet_url = "https://docs.google.com/spreadsheets/d/1Sahu1Q3uaP9JSNpAYjw0YUAM3bxFUPvfewN_kixocww/edit#gid=0"
...
if 'EWYL Status' not in priority_cases_df.columns:
    priority_cases_df['EWYL Status'] = ''
if 'Category' not in priority_cases_df.columns:
    priority_cases_df['Category'] = ''
if 'EWYL Mentor' not in priority_cases_df.columns:
    priority_cases_df['EWYL Mentor'] = ''

# Map Group Name to additional columns from Mastersheet
for index, row in priority_cases_df.iterrows():
    group_name = row['EWYL Group Name']
    master_match = mastersheet_df[mastersheet_df['EWYL Group Name'].str.contains(re.escape(group_name), case=False, na=False)]
    if not master_match.empty:
        master_data = master_match.iloc[0]
        priority_cases_df.at[index, 'EWYL Status'] = master_data['EWYL Status']
        priority_cases_df.at[index, 'Category'] = master_data['Category']
        priority_cases_df.at[index, 'EWYL Mentor'] = master_data['EWYL Mentor']
...
def update_priority_cases(priority_cases_df, daily_report_df, date_str):
    ...
    for index, row in priority_cases_df.iterrows():
        student_name = row['EWYL Group Name']
...
sorted_columns = ['EWYL Group Name', 'EWYL Status', 'Category', 'EWYL Mentor', 'Total STU', 'Total EMP', 'Days STU', 'Days EMP'] + date_columns_sorted
